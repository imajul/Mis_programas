...
void onRxTimeOutCallback( TimerHandle_t xTimer )
{
    ...
    if ( selectedUart->rxLen <= PACKET_SIZE ) //si la cantidad de bytes es correcta
    {
    	...
    	selectedUart->rxChecksum = str_to_checksum(selectedUart->rxBlock,selectedUart->rxLen);


        //Guardo en el primer byte del bloque la longitud del paqute (le descueto el CRC)
        //selectedUart->rxBlock[0] = selectedUart->rxLen - 1;
